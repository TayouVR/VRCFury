                ...
                try {
                    ...
                    
                    {
                        if (socket.addChannelToggle) {
                            ...
                            menu.NewMenuToggle($"{spsOptions.GetOptionsPath()}/<b>Channels<\\/b>\n<size=20>Toggle DPS Channels/{name}", param);

                            string frontLightPath = bakeResult.lights.Children().First(child => child.name == "Front").GetPath(avatarObject);
                            string rootLightPath = bakeResult.lights.Children().First(child => child.name == "Root").GetPath(avatarObject);
                            ...
                            dpsChannel0Clip.SetCurve(
                                ...
                                new AnimationCurve(new Keyframe(0,
                                    VRCFuryHapticSocketEditor.GetLightRange(true, Channel.Default)))
                            );
                            dpsChannel0Clip.SetCurve(
                                EditorCurveBinding.FloatCurve(rootLightPath, typeof(Light), "m_Range"), 
                                new AnimationCurve(new Keyframe(0,
                                    VRCFuryHapticSocketEditor.GetLightRange(false, Channel.Default, socket.addLight)))
                            );
                            dpsChannel1Clip.SetCurve(
                                EditorCurveBinding.FloatCurve(frontLightPath, typeof(Light), "m_Range"), 
                                new AnimationCurve(new Keyframe(0, 
                                    VRCFuryHapticSocketEditor.GetLightRange(true, Channel.LegacyDPSChannel1)))
                            );
                            dpsChannel1Clip.SetCurve(
                                EditorCurveBinding.FloatCurve(rootLightPath, typeof(Light), "m_Range"), 
                                new AnimationCurve(new Keyframe(0, VRCFuryHapticSocketEditor.GetLightRange(false, Channel.LegacyDPSChannel1, socket.addLight)))
                            );
                        ...
                        socketRewritesToDo.Add(new SocketRewriteToDo{plugObject = socket.owner(), bakeRoot = bakeResult.worldSpace, addLight = socket.addLight}); 
                    } 
