...
void sps_light_parse(float range, half4 color, out int type, int channel) {
	...
	if (channel == 1) { // legacy DPS channel 1
		if (secondDecimal == 3) type = SPS_TYPE_HOLE;
		if (secondDecimal == 4) type = SPS_TYPE_RING_TWOWAY;
		if (secondDecimal == 6) type = SPS_TYPE_FRONT;
	} else if (channel == 0) { // Default Channel
...
void sps_light_search(
	...
	float3 lightLocalPos[4];
	{
		for(int i = 0; i < 4; i++) {
			...
			sps_light_parse(range, unity_LightColor[i], lightType[i], _SPS_Channel);
